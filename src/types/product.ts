...
export interface AffiliateLink {
  ...
  /**
   * 楽天アフィリエイト「画像のみ」リンクの生成HTML。rakutenHtml（テキストのみ）とは別に、
   * 楽天アフィリエイトツールが発行した画像アフィリエイトタグをそのまま保存する。
   * 改変・画像URLの抽出流用はせず、rakutenHtmlと同じ「そのまま保存・そのまま描画」方式を取る。
   * 画像自体が楽天の商品ページへのアフィリエイトリンクを兼ねるため、購入導線がある文脈でのみ使用する。
   * 未提供の場合はnull。
   *
   * 運用ルール：楽天画像リンクは、同じrakutenHtml（購入ボタン）と同一店舗を原則とする。
   * ただし、同一商品の正規販売ページであることを個別に照合済みであり、かつ楽天アフィリエイト
   * から正式に生成された画像リンクである場合に限り、別店舗のリンクを許容する
   * （例：SwitchBot S20は購入ボタンがSwitchBot公式店、画像はコジマ楽天市場店。
   * 　本体・充電ドックの寸法が完全一致することを確認済み）。
   */
