 ...
 * ranking記事：comparisons.jsonのtitle/slug/updatedAtと、axisScoreProfiles.jsonのpersona名・重みだけから
 * 組み立てる（記事本文・要約文の新規追加はしない）。
 * guide記事：ランキング・personaを持たないため、固定の紹介文（新しい商品事実は含まない）を表示する。
 */
export function ArticleCard({ comparison }: { comparison: Comparison }) {
  const isGuide = comparison.articleType === "guide";
  const profile = !isGuide && comparison.personaProfileId ? getProfile(comparison.personaProfileId) : null;
  const highlightAxis = profile ? getHighlightAxis(profile) : null;
  ...
  return (
    ...
    >
      <p className="mb-2 text-xs font-bold uppercase tracking-wide text-brand-accent2">
        {isGuide ? "選び方ガイド" : "比較記事"}
      </p>
      ...
      <p className="mb-4 flex-1 text-sm text-brand-inkSoft">
        {isGuide || !profile ? (
          "5つのAXISから、自分が何を重視すべきかを整理できるガイド記事です。"
        ) : (
